...
(function () {
    ...
    let svgNS = "http://www.w3.org/2000/svg", xlinkNS = "http://www.w3.org/1999/xlink";
    let version = "0.24.0",
        ...
        isSVG = isBrowser && (document.documentElement.localName === "svg" && document.documentElement.namespaceURI === svgNS),
    ...
    function newContext(contextName) {
        ...
        function makeContextModuleFunc(func, relModuleMap, enableBuildCallback) {
            return function () {
                ...
                let args = [].concat(aps.call(arguments, 0)), lastArg;
        ...
        function makeRequire(relModuleMap, enableBuildCallback) {
            let modRequire = makeContextModuleFunc(context.require, relModuleMap, enableBuildCallback);
        ...
        function updateNormalizedNames(pluginName) {
            ...
            let oldFullName, oldModuleMap, moduleMap, fullName, callbacks,
        ...
        function main(inName, depArray, callback, relModuleMap) {
            let moduleMap = makeModuleMap(inName, relModuleMap),
                name = moduleMap.name,
                fullName = moduleMap.fullName,
                uniques = {},
                manager = {
        ...
        function jQueryCheck(jqCandidate) {
            if (!context.jQuery) {
                let $ = jqCandidate || (typeof jQuery !== "undefined" ? jQuery : null);
                if ($ && "readyWait" in $) {
                    context.jQuery = $;
        ...
        function checkLoaded() {
            let waitInterval = config.waitSeconds * 1000,
        ...
        function callPlugin(pluginName, dep) {
            ...
            load.fromText = function (moduleName, text) {
                /*jslint evil: true */
                let hasInteractive = useInteractive;
                ...
                if (defined[pluginName]) {
                    ...
                    if (!pluginsQueue[pluginName]) {
                        ...
                        (managerCallbacks[pluginName] = [])).push({
                            onDep: function (name, value) {
                                if (name === pluginName) {
                                    let i, oldModuleMap, ary = pluginsQueue[pluginName];
        ...
        context = {
            ...
            configure: function (cfg) {
                let paths, prop, packages, pkgs, packagePaths, requireWait;
            ...
            takeGlobalQueue: function () {
                ...
                if (globalDefQueue.length) {
                    ...
                    //local let ref to defQueue, so cannot just reassign the one
                    //on context.
            ...
            nameToUrl: function (moduleName, ext, relModuleMap) {
                let paths, pkgs, pkg, pkgPath, syms, i, parentModule, url,
    ...
    req.load = function (context, moduleName, url) {
        let contextName = context.contextName,
    ...
    req.onScriptLoad = function (evt) {
        ...
        let node = evt.currentTarget || evt.srcElement, contextName, moduleName,
            context;

        console.log('in onScriptLoad')
        //debugger;
        if (evt.type === "load" || readyRegExp.test(node.readyState)) {
            ...
            //in this file.
            if (node.detachEvent && !isOpera //&& !isSVG
			) {
    ...
    function createScriptNode(url,contextName,moduleName){
        let node;
        if(isBrowser && isSVG){
            node = document.createElementNS(svgNS,"script");
            ...
            node.setAttributeNS(null,"async", !s.skipAsync[url]);
            node.setAttributeNS(null,"data-requirecontext", contextName);
            node.setAttributeNS(null,"data-requiremodule", moduleName);
        }else{
    ...
    req.attach = function (url, contextName, moduleName, callback, type) {
        let node, loaded, context;
        if (isBrowser) {
            //In the browser so use a script tag
            callback = callback || req.onScriptLoad;

            let node = createScriptNode(url,contextName, moduleName);
            ...
            if (node.attachEvent && !isOpera //&& !isSVG
			) {
            ...
            //svg
            if(isSVG){
                node.setAttributeNS(xlinkNS,"href",url);
            }
            else{
                node.src = url;
            }
            ...
            //svg
            if(isSVG){
                console.log('before attaching script node')
                document.documentElement.appendChild(node);
                console.log('after attaching script node')
            }else{
                if (baseElement) {
    ...
    req.callReady = function () {
        let callbacks = s.readyCalls, i, callback, contexts, context, prop;
