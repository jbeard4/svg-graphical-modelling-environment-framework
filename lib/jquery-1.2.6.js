(function () {
	...
	let jQuery = (window.jQuery = window.$ = function (selector, context) {
	...
	jQuery.fn = jQuery.prototype = {
			...
			if (typeof selector == 'string') {
				...
				if (match && (match[1] || !context)) {
					...
					// HANDLE: $("#id")
						...
						let elem = document.getElementById(match[3]);
		...
		find: function (selector) {
			let elems = jQuery.map(this, function (elem) {
			...
			if (events === true)
				this.find('*')
					...
					.each(function (i) {
						...
						let events = jQuery.data(this, 'events');

						for (let type in events)
							for (let handler in events[type])
								jQuery.event.add(
									clone[i],
									type,
									events[type][handler],
		...
		data: function (key, value) {
			...
			if (value === undefined) {
				let data = this.triggerHandler('getData' + parts[1] + '!', [
					parts[0],
				]);
		...
		domManip: function (args, table, reverse, callback) {
			...
			return this.each(function () {
				...
				jQuery.each(elems, function () {
					let elem = clone ? jQuery(this).clone(true)[0] : this;
	...
	jQuery.extend({
		...
		swap: function (elem, options, callback) {
			...
			for (let name in options) elem.style[name] = old[name];
		},
		...
		curCSS: function (elem, name, force) {
			...
			function color(elem) {
				...
				let ret = defaultView.getComputedStyle(elem, null);
		...
		clean: function (elems, context) {
			...
			jQuery.each(elems, function (i, elem) {
				...
				if (typeof elem == 'string') {
					...
					let tags = jQuery.trim(elem).toLowerCase(),
		...
		attr: function (elem, name, value) {
			...
			let notxml = !jQuery.isXMLDoc(elem),
			...
			if (elem.tagName) {
				...
				let special = /href|src|style/.test(name);
	...
	let styleFloat = jQuery.browser.msie ? 'styleFloat' : 'cssFloat';
	...
	jQuery.extend({
		...
		multiFilter: function (expr, elems, not) {
			...
			while (expr && expr != old) {
				old = expr;
				let f = jQuery.filter(expr, elems, not);
		...
		find: function (t, context) {
			...
			while (t && last != t) {
				...
				if (m) {
					...
					for (let i = 0; ret[i]; i++)
						for (let c = ret[i].firstChild; c; c = c.nextSibling)
							if (
								c.nodeType == 1 &&
								(nodeName == '*' ||
					...
					if ((m = re.exec(t)) != null) {
						...
						for (let j = 0, rl = ret.length; j < rl; j++) {
							let n =
								m == '~' || m == '+'
									? ret[j].nextSibling
									: ret[j].firstChild;
				...
				if (t && !foundToken) {
					...
					if (!t.indexOf(',')) {
						...
						if (
							...
							let oid = elem.getElementById(m[2]);
							...
							ret = r =
								oid && (!m[3] || jQuery.nodeName(oid, m[3]))
									? [oid]
									: [];
						} else {
		...
		classFilter: function (r, m, not) {
			...
			let tmp = [];
			for (let i = 0; r[i]; i++) {
				let pass = (' ' + r[i].className + ' ').indexOf(m) >= 0;
	...
	jQuery.event = {
		...
		remove: function (elem, types, handler) {
			...
			let events = jQuery.data(elem, 'events'),
				ret,
				index;
			...
			if (events) {
					...
					jQuery.each(types.split(/\s+/), function (index, type) {
						...
						if (events[type]) {
							...
							if (!ret) {
								if (
									!jQuery.event.special[type] ||
									jQuery.event.special[type].teardown.call(
										elem
									) === false
								) {
				...
				if (!ret) {
					let handle = jQuery.data(elem, 'handle');
		...
		handle: function (event) {
			...
			let val, ret, namespace, all, handlers;
	...
	jQuery.fn.extend({
		...
		one: function (type, data, fn) {
			let one = jQuery.event.proxy(fn || data, function (event) {
		...
		triggerHandler: function (type, data, fn) {
			return (
				this[0] && jQuery.event.trigger(type, data, this[0], false, fn)
			);
		},
		...
		toggle: function (fn) {
			...
			return this.click(
				jQuery.event.proxy(fn, function (event) {
					...
					return (
						args[this.lastToggle++].apply(this, arguments) || false
					);
				})
			);
		},
	...
	let withinElement = function (event, elem) {
	...
	jQuery.extend({
		...
		ajax: function (s) {
			...
			if (s.cache === false && type == 'GET') {
				...
				let ret = s.url.replace(/(\?|&)_=.*?(&|$)/, '$1_=' + ts + '$2');
			...
			let onreadystatechange = function (isTimeout) {
			...
			if (s.async) {
				...
				let ival = setInterval(onreadystatechange, 13);
		...
		httpNotModified: function (xhr, url) {
			try {
				let xhrRes = xhr.getResponseHeader('Last-Modified');
		...
		animate: function (prop, speed, easing, callback) {
			...
			return this[optall.queue === false ? 'each' : 'queue'](function () {
				...
				jQuery.each(prop, function (name, val) {
					let e = new jQuery.fx(self, opt, name);
	...
	let queue = function (elem, type, array) {
		if (elem) {
			type = type || 'fx';

			let q = jQuery.data(elem, type + 'queue');
	...
	jQuery.fx.prototype = {
		...
		cur: function (force) {
			...
			let r = parseFloat(jQuery.css(this.elem, this.prop, force));
			return r && r > -10000
				? r
		...
		step: function (gotoEnd) {
			...
			if (gotoEnd || t > this.options.duration + this.startTime) {
				...
				let done = true;
				for (let i in this.options.curAnim)
				...
				if (done) {
					...
					if (this.options.hide || this.options.show)
						for (let p in this.options.curAnim)
							jQuery.attr(
								this.elem.style,
								p,
								this.options.orig[p]
	...
	jQuery.fn.extend({
		...
		offsetParent: function () {
			let offsetParent = this[0].offsetParent;
			while (
				offsetParent &&
