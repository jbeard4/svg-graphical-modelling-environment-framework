 ...
 **/
define(['helpers', 'behaviours'], function (h, behaviours) {
	let api = {
		remove: function () {
			...
			let thisAndDescendants = descendants.concat(this);
			...
			thisAndDescendants.forEach(function (node) {
				...
				let splinesTargetingNode = allSplinesInConstraintGraph.filter(
					function (c) {
						//flatten out dests
						let allNodeAttrs = c.dest.reduce(function (a, b) {
							return a.concat(b.nodeAttrs);
						}, []);

						let nodeAttrsTargetingThis = allNodeAttrs.filter(
							function (nodeAttr) {
				...
				let splinesToRemove = splinesTargetingNode.map(function (c) {
					//get source node
					return c.source.node.pathRef;
				});

				let uniqueSplinesToRemove = h.unique(splinesToRemove);

				//console.log("uniqueSplinesToRemove",uniqueSplinesToRemove);

				uniqueSplinesToRemove.forEach(function (spline) {
					spline.remove();
				});

				//remove constraints in which this is the source from the constraint graph
				//we also have a case handling path segments.
				let constraintsWithThisAsSource = this.env.constraintGraph.filter(
					function (c) {
						return (
							c.source.node === node ||
							(c.source.node.pathRef &&
								c.source.node.pathRef === node)
						);
					}
				);
				...
				this.env.constraintGraph.forEach(function (c) {
					c.dest.forEach(function (nodeAttrExpr) {
						let nodeAttrsToRemove = nodeAttrExpr.nodeAttrs.filter(
							function (nodeAttr) {
								return (
									nodeAttr.node === node ||
									(nodeAttr.node.pathRef &&
										nodeAttr.node.pathRef === node)
								);
							}
						);
					...
					let destsToRemove = c.dest.filter(function (nodeAttrExpr) {
				...
				let constraintsToRemove = this.env.constraintGraph.filter(
					function (c) {
