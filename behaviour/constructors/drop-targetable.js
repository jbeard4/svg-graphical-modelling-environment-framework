 ...
 **/
define(['c', 'helpers', 'behaviour/constructors/highlightable'], function (
	cm,
	h,
	setupHighlightable
) {
	...
	function setupDropTarget(env, icon, spacing, setupWrapConstraints) {
		...
		this.dropShape = function (shape) {
			...
			let m2 = shape.getTransformToElement(this);

			let tl = shape.transform.baseVal;
			let t = tl.numberOfItems
				? tl.getItem(0)
				: shape.ownerSVGElement.createSVGTransform();
			let m = t.matrix;
			t.setMatrix(m2);
			tl.initialize(t);
			...
			[
				...
				'thisYConstraint',
				'thisWidthConstraint',
				'thisHeightConstraint',
			].forEach(function (prop) {
				let constraint = p[prop];
				...
				if (constraint) {
					...
					constraint.dest.forEach(function (nodeAttrExpr) {
						nodeAttrExpr.nodeAttrs = nodeAttrExpr.nodeAttrs.filter(
							function (nodeAttr) {
					...
					constraint.dest = constraint.dest.filter(function (
						nodeAttrExpr
					) {
			...
			if (setupWrapConstraints) {
				if (!icon.thisXConstraint) {
					icon.thisXConstraint = cm.Constraint(
						cm.NodeAttr(this, 'x'),
						[
							...
							cm.NodeAttrExpr(
								shape,
								'x',
								cm.dec(spacing.leftPadding)
							),
						],
						Math.min
					);

					icon.thisYConstraint = cm.Constraint(
						cm.NodeAttr(this, 'y'),
						[
							...
							cm.NodeAttrExpr(
								shape,
								'y',
								cm.dec(spacing.topPadding)
							),
						],
						Math.min
					);

					icon.thisWidthConstraint = cm.Constraint(
						...
						function (thisX, thisRightX) {
							...
							args = args.slice(2);
							/*jsl:ignore*/
							let rightXArgs = args.map(function (shapeRightX) {
								return shapeRightX - thisX;
							});
							/*jsl:end*/
							let rightX = Math.max.apply(this, rightXArgs);
							let rightXPlusPadding =
								rightX + spacing.rightPadding;
							rightXPlusPadding = Math.max(
								rightXPlusPadding,
								thisRightX - thisX
							);
							return rightXPlusPadding >= spacing.minWidth
								? rightXPlusPadding
								: spacing.minWidth;
						}
					);

					icon.thisHeightConstraint = cm.Constraint(
						...
						function (thisY, thisBottomY) {
							...
							args = args.slice(2);
							/*jsl:ignore*/
							let bottomYArgs = args.map(function (shapeBottomY) {
								return shapeBottomY - thisY;
							});
							/*jsl:end*/
							let bottomY = Math.max.apply(this, bottomYArgs);
							let bottomYPlusPadding =
								bottomY + spacing.leftPadding;
							bottomYPlusPadding = Math.max(
								bottomYPlusPadding,
								thisBottomY - thisY
							);

							return bottomYPlusPadding >= spacing.minHeight
								? bottomYPlusPadding
								: spacing.minHeight;
						}
					);
					...
					icon.thisWidthConstraint.dest.push(
						cm.NodeAttrExpr(shape, ['x', 'width'], cm.sum)
					);
					icon.thisHeightConstraint.dest.push(
						cm.NodeAttrExpr(shape, ['y', 'height'], cm.sum)
					);
				}
			}
