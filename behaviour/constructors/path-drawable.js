...
define([
	...
	function getSourceConstraintFunction(xOrY) {
		return function (x0, y0, toX, toY, toWidth, toHeight) {
			let x1 = toX + toWidth / 2;
			let y1 = toY + toHeight / 2;

			let p1 = new Point2D(x0, y0),
			...
			let inter = Intersection.intersectLineRectangle(p1, p2, r1, r2);

			let point = inter.points.pop();
	...
	function getSimpleConstraintFunction(xOrY, isSource) {
		return function (
			...
			let x0 = fromX + fromWidth / 2;
			let y0 = fromY + fromHeight / 2;
	...
	let drawPathBehaviourAPI = {
		willPathBeEmptyAfterRemovingNextPoint: function () {
			return this.path.pathSegList.numberOfItems == 2;
		},

		rollbackPoint: function () {
			this.path.pathSegList.removeItem(
				this.path.pathSegList.numberOfItems - 2
			);
		},

		setEndPoint: function (x, y) {
			let segList = this.path.pathSegList;
		...
		createNewQuadraticSegment: function (x, y, x1, y1) {
			let n = this.path.createSVGPathSegCurvetoQuadraticAbs(x, y, x1, y1);
		...
		createNewLineSegment: function (x, y) {
			let n = this.path.createSVGPathSegLinetoAbs(x, y);
			this.path.pathSegList.appendItem(n);
		...
		addControlPoint: function (x, y, mirror) {
			...
			if (numItems > 1) {
				if (endSeg.x2 === undefined && endSeg.x1 === undefined) {
					...
					newSeg = this.path.createSVGPathSegCurvetoQuadraticAbs(
						endSeg.x,
						endSeg.y,
						x,
						y
					...
					//let newSegX2 = x + 2*( endSeg.x1 - x );
					//let newSegY2 = y + 2*( endSeg.y1 - y );
					newSeg = this.path.createSVGPathSegCurvetoCubicAbs(
						endSeg.x,
						endSeg.y,
						endSeg.x1,
						endSeg.y1,
		...
		setTarget: function (target) {
			...
			for (
				let i = 0, l = this.path.pathSegList.numberOfItems;
				i < l;
				i++
			) {
				let pathSeg = this.path.pathSegList.getItem(i);
				let thickPathSeg = this.thickPath.pathSegList.getItem(i);
			...
			[this.sourceConstraintX, this.sourceConstraintY].forEach(function (
				c
			) {
			...
			let segAfterStartSeg = segList.getItem(1);

			let getTargetConstraintFunction,
			...
			let targetIsPath = target.localName === 'path';
			let useAdvancedLayout =
				numItems > 2 ||
				segAfterStartSeg.x1 !== undefined ||
				targetIsPath;
			...
			if (useAdvancedLayout) {
				afterStartSegPropStr =
					segAfterStartSeg.x1 !== undefined ? '1' : '';
				...
				if (targetIsPath) {
					endDepList = [
						cm.NodeAttrExpr(
							segBeforeEndSeg,
							'x' + beforeEndSegPropStr
						),
						cm.NodeAttrExpr(
							segBeforeEndSeg,
							'y' + beforeEndSegPropStr
						),
					...
					getTargetConstraintFunction = function (xOrY) {
						return function (x0, y0, segList) {
							let o = getNearestPointOnPath(segList, {
								x: x0,
								y: y0,
							});
					...
					getTargetConstraintFunction = function (xOrY) {
						return function (x0, y0, toX, toY, toWidth, toHeight) {
							...
							let inter = Intersection.intersectLineRectangle(
								p1,
								p2,
								r1,
								r2
				...
				startDepList = [
					cm.NodeAttrExpr(
						segAfterStartSeg,
						'x' + afterStartSegPropStr
					),
					cm.NodeAttrExpr(
						segAfterStartSeg,
						'y' + afterStartSegPropStr
					),
				...
				this.sourceConstraintX = cm.Constraint(
					cm.NodeAttr(startSeg, 'x'),
					startDepList,
					getSourceConstraintFunction('x', true)
				);

				this.sourceConstraintY = cm.Constraint(
		...
		unsetTarget: function () {
			...
			if (this.target.localName !== 'path') {
				...
				let constraintsToRemove = this.thickPathBindingConstraints.concat(
					[
						this.sourceConstraintX,
						this.sourceConstraintY,
						this.targetConstraintX,
				...
				this.env.constraintGraph.push(
					this.originalSourceConstraintX,
					this.originalSourceConstraintY
				);
			}
		},
	};
	...
	function setupDrawPath(env, source, path) {
		...
		this.originalSourceConstraintX = this.sourceConstraintX = cm.Constraint(
			cm.NodeAttr(startSeg, 'x'),
		...
		this.originalSourceConstraintY = this.sourceConstraintY = cm.Constraint(
			cm.NodeAttr(startSeg, 'y'),
			cm.NodeAttrExpr(this.source, 'bbox'),
			function (sourceBBox) {
