...
define([
	...
	return function (env, x, y) {
		...
		let nameContainerRect = env.svg.rect(icon, 0, 0, 1, 1);
		let nameText = env.svg.text(icon, 0, 0, 'Package');

		let classContainerRect = env.svg.rect(
			icon,
			x,
			y,
			PACKAGE_MIN_WIDTH,
		...
		let classContainerRectEastResizeHandle = env.svg.line(
			icon,
			x + PACKAGE_MIN_WIDTH,
			y,
			x + PACKAGE_MIN_WIDTH,
			y + PACKAGE_MIN_HEIGHT
		);

		let classContainerRectSouthResizeHandle = env.svg.line(
			icon,
			x,
			y + PACKAGE_MIN_HEIGHT,
			x + PACKAGE_MIN_WIDTH,
		...
		let classContainerRectSouthEastResizeHandle = env.svg.line(
			icon,
			x + PACKAGE_MIN_WIDTH - 5,
			y + PACKAGE_MIN_HEIGHT - 5,
			x + PACKAGE_MIN_WIDTH,
			y + PACKAGE_MIN_HEIGHT
		); //hardcoded 5 pixels

		let children = [icon, nameContainerRect, nameText, classContainerRect];
		...
		env.constraintGraph.push(
			...
			cm.Constraint(
				...
				[
					...
					cm.NodeAttrExpr(nameContainerRect, 'height'),
				],
		...
		let resizeHandleKwArgs = {
			associatedRect: classContainerRect,
		};

		resizable.setupResizableEast.call(
			classContainerRectEastResizeHandle,
			env,
			resizeHandleKwArgs
		);
		resizable.setupResizableSouth.call(
			classContainerRectSouthResizeHandle,
			env,
			resizeHandleKwArgs
		);
		resizable.setupResizableSouthEast.call(
			classContainerRectSouthEastResizeHandle,
			env,
			resizeHandleKwArgs
		);
