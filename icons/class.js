...
define(	
	...
	function(cm,setupSelectable,setupRemoveable,setupArrowSource,setupArrowTarget,setupDraggable,setupTextEditable){
		return function(env,x,y){
			...
			var nameContainerRect = env.svg.rect(icon,x,y,1,1);
			var nameText = env.svg.text(icon,0,0,"Class");	//we really shouldn't set x and y here... maybe use different api?

			nameContainerRect.id = "nameContainerRect";
			nameText.id = "nameText";

			var attributeListRect = env.svg.rect(icon,0,0,100,10);	//set an initial height
			attributeListRect.id = "attributeListRect";

			var NEW_ATTRIBUTE_BUTTON_RADIUS = 5; 

			var newAttributeButton = env.svg.circle(icon,0,0,NEW_ATTRIBUTE_BUTTON_RADIUS);
			...
			newAttributeButton.addEventListener("mousedown",function(e){
				...
				var newAttribute = env.svg.text(icon,x,y,"+attributeName : attributeType");	
				...
				env.hookElementEventsToStatechart(newAttribute,["mousedown"],false);
